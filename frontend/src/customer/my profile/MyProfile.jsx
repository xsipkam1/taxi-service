...
export default function MyProfile() {
    ...
    const loadDetails = async () => {
        const result = await axios.get("/customer/info", { headers: { Authorization: `Bearer ${userToken}` } });
    ...
    const handleConfirmDelete = async () => {
        ...
        const result = await axios.delete("/customer/delete", { headers: { Authorization: `Bearer ${userToken}` } });
    ...
    const handleConfirmDeletePic = async () => {
        ...
        const result = await axios.delete("/customer/delete-pic", { headers: { Authorization: `Bearer ${userToken}` } });
        if (result.status === 200) {
    ...
    const changePassword = async (values) => {
        try {
            const userToken = localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user')).token : null;
            const response = await axios.put("/customer/change-password", values, { headers: { Authorization: `Bearer ${userToken}` } });
    ...
    const uploadImage = async () => {
        ...
        setImageSelected();
        const result2 = await axios.post("/customer/upload-pic", values, { headers: { Authorization: `Bearer ${userToken}` } });
    ...
    return (
        <section className='container-md d-flex personal-profile'>
            ...
            {activeTab == "DETAILY" &&
                <div className="right-col">
                    ...
                    <div className="personal-details">
                        ...
                        <div className="detail-line">
                            <span className="label">Počet objednaných jázd:</span>
                            <span className="value">{details?.pendingAmount}</span>
                        </div>
                        <div className="detail-line">
                            <span className="label">Počet uskutočnených jázd:</span>
                            <span className="value">{details?.completedAmount}</span>
                        </div>
                        <div className="detail-line">
                            <span className="label">Počet zamietnutých jázd:</span>
                            <span className="value">{details?.deniedAmount}</span>
                        </div>
                        <div className="detail-line">
                            <span className="label">Dátum registrácie:</span>
                            <span className="value">{formatDate(details?.registrationDate)}</span>
                        </div>
                    </div>
                </div>
            }
            ...
            <ResponseModal
                ...
                handleClose={() => { setResponseModalPicShow(false); window.location.href = '/customer/myprofile';}}
                title="ÚSPEŠNE VYMAZANÉ"
            ...
            <ResponseModal
                ...
                handleClose={() => { setImageUploadedSuccess(false); window.location.href = '/customer/myprofile'; }}
