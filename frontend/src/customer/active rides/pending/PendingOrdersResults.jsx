...
export default function PendingOrdersResults({ orders, reload }) {
    ...
    const filteredOrders = ordersList.filter((order) => {
        ...
        return (
            ...
            String((order.drivers[0]?.cost*order.distance).toFixed(2)).includes(searchValue) ||
    ...
    return (
        <>
            {orders.length === 0 ? (
                <h2 className="text-center">MOMENTÁLNE ŽIADNE OBJEDNÁVKY ČAKAJÚCE NA POTVRDENIE VODIČOM</h2>
            ) : (
                <>
                    ...
                    {filteredOrders.map((order) => (
                        <PendingOrderResult key={order.id} order={order} reload={reload}/>
                    ))}
                </>
            )}
        </>
