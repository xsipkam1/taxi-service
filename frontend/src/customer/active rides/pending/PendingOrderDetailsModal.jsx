...
export default function PendingOrderDetailsModal({ show, handleClose, order }) {
                    ...
                    <div className="right-col align-self-center w-100">
                        ...
                        {order.type === 'driver' && (
                            <div>
                                ...
                                <div id="carouselExampleIndicators" className="carousel slide order-modal-profile-pic mx-auto">
                        ...
                        {order.type === 'car' && (
                            <div>
                                <div className='d-flex justify-content-center mb-3'>
                                    <h3 className='m-0'>{`${order.drivers[0].car.name}`}</h3>
                                </div>
                                <div className="carousel slide order-modal-profile-pic mx-auto">
                                    <div className="carousel-inner">
                                        {order.drivers[0].car.carPictureUrl && !carImageLoadError ? (
                                            <Image
                                                cloudName={import.meta.env.VITE_CLOUD_PRESET}
                        ...
                        {order.type === 'car' && (
                            <div className='d-flex justify-content-center mt-3'>
                                <span className="text-center"><i className="bi bi-people"></i> Počet vodičov, ktorý obdržali žiadosť: {`${order.drivers.length + order.deniedDrivers.length}`}</span>
                            </div>
                        )}
                        {order.type === 'quick' && (
                            <div className='d-flex justify-content-center mt-3'>
                        ...
                        <div className="personal-details">
                            ...
                            {order.type === 'car' && (
                                <div className="detail-line mb-0">
                                    ...
                                    <span className="value">{`${lowestCost}€`}</span>
                                </div>
                            )}
                            {order.type === 'car' && (
                                <div className="detail-line mb-0">
                                    <span className="label fw-light">Najväčšia cena/km:</span>
                                    <span className="value">{`${highestCost}€`}</span>
                                </div>
                            )}
                            ...
                            <div className="detail-line mb-0">
                                ...
                                {order.type === 'car' && (
                                    <h2 className="value"><strong>{`${(lowestCost === highestCost ? (lowestCost * order.distance).toFixed(2) : `${(lowestCost * order.distance).toFixed(2)}€-${(highestCost * order.distance).toFixed(2)}€`)}`}</strong></h2>
                                )}
                                {order.type === 'quick' && (
                                    <h2 className="value"><strong>{`${(lowestCost === highestCost ? (lowestCost * order.distance).toFixed(2) : `${(lowestCost * order.distance).toFixed(2)}€-${(highestCost * order.distance).toFixed(2)}€`)}`}</strong></h2>
                                )}
