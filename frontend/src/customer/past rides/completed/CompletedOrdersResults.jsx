import CompletedOrderResult from './CompletedOrderResult';
import { useState } from 'react';

export default function CompletedOrdersResults({ orders }) {
    ...
    return (
        <>
            {orders.length === 0 ? (
                <h2 className="text-center">MOMENTÁLNE ŽIADNE DOKONČENÉ JAZDY</h2>
            ) : (
                <>
                    <div className="search-bar-container d-flex filter-options align-items-center">
                        ...
                        <div className="dropdown w-100 d-flex m-0 mb-3">
                            ...
                            <select className="form-select" aria-label="Dropdown" onChange={handleSortingChange} value={sortingOption}>
                                <option value="1">Dátum (zostupne)</option>
                                <option value="2">Dátum (vzostupne)</option>
                                <option value="3">Cena (zostupne)</option>
                                <option value="4">Cena (vzostupne)</option>
                            </select>
                        </div>
                    </div>

                    {filteredOrders.map((order) => (
                        <CompletedOrderResult key={order.id} order={order}/>
                    ))}
                </>
            )}
        </>
