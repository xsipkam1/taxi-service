...
const DriverOrdersResults = ({ orders, textEmpty, resultComponent: ResultComponent, reload }) => {
    ...
    const filteredOrders = ordersList.filter((order) => {
        ...
        return (
            ...
            order.customer.fname.toLowerCase().includes(searchValue) ||
            order.customer.lname.toLowerCase().includes(searchValue)
        );
    });
    ...
    switch (sortingOption) {
        ...
        case 2:
            ...
            break;
        case 3:
            filteredOrders.sort((a, b) => a.driverCost * a.distance - b.driverCost * b.distance);
            break;
        case 4:
            filteredOrders.sort((a, b) => b.driverCost * b.distance - a.driverCost * a.distance);
            break;
        case 5:
            filteredOrders.sort((a, b) => parseFloat(a.distance) - parseFloat(b.distance));
            break;
        case 6:
            filteredOrders.sort((a, b) => parseFloat(b.distance) - parseFloat(a.distance));
            break;
        default:
            break;
    }
    ...
    return (
        <>
            {orders.length === 0 ? (
                <h2 className="text-center">{textEmpty}</h2>
            ) : (
                <>
                    <div className="search-bar-container d-flex filter-options align-items-center">
                        ...
                        <div className="dropdown w-100 d-flex m-0 mb-3">
                            ...
                            <select className="form-select" aria-label="Dropdown" onChange={handleSortingChange} value={sortingOption}>
                                ...
                                <option value="5">Dĺžka v km (vzostupne)</option>
                                <option value="6">Dĺžka v km (zostupne)</option>
                            </select>
                        </div>
                    </div>

                    {filteredOrders.map((order) => (
                        <ResultComponent key={order.id} order={order} reload={reload}/>
                    ))}
                </>
            )}
        </>
...
export default DriverOrdersResults;
